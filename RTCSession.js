...
var debug = require('debug')('JsSIP:RTCSession', true);
var debugerror = require('debug')('JsSIP:ERROR:RTCSession', true);
...
function createLocalDescription(type, onSuccess, onFailure, constraints) {
  ...
  function createSucceeded (desc) {
    connection.onicecandidate = function (event, candidate) {
      if (!candidate) {
        createLocalDescription.call(self, type, onSuccess, onFailure, constraints);
      }
    }
    ...
    } else if (connecting.iceGatheringState === 'complete') {
      if (type === 'answer') {
